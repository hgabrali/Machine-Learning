...
# 🔬 Kümeleme (Clustering): DBSCAN Açıklaması

DBSCAN (Density-Based Spatial Clustering of Applications with Noise), özellikle karmaşık veri yapılarını anlamak için çok önemli, yoğunluğa dayalı bir kümeleme algoritmasıdır.

## DBSCAN Nedir?

**DBSCAN = Density-Based Spatial Clustering of Applications with Noise** (Gürültüye Sahip Uygulamalar İçin Yoğunluğa Dayalı Uzamsal Kümeleme).

* Diğer kümeleme algoritmalarının (örneğin K-Means) aksine, DBSCAN **rastgele şekilli kümeleri** bulmakta harikadır.
* En büyük avantajı, kümeler arasına düşen veya herhangi bir yoğun bölgeye ait olmayan noktaları otomatik olarak **gürültü (noise)** veya **aykırı değer (outlier)** olarak etiketlemesidir.

---

## DBSCAN Nasıl Çalışır?

DBSCAN, kümeleri tanımlamak için veri noktalarını üç ana kategoriye ayırır. Bu sınıflandırma, belirlediğimiz iki temel parametreye bağlıdır: `Eps` ve `MinPts`.

<img width="690" height="465" alt="image" src="https://github.com/user-attachments/assets/d50b79dc-29d2-4992-ada7-1c7ba132f84a" />


### 1. Temel Parametreler

* **Eps (Epsilon):** Bir nokta ile komşusu arasında olabilecek **maksimum mesafedir**. Bu mesafedeki tüm noktalar, bir noktanın "mahallesini" (neighborhood) oluşturur.
* **MinPts (Minimum Points):** Bir bölgenin **yoğun bir bölge (küme)** olarak kabul edilmesi için gerekli olan **minimum nokta sayısıdır** (Eps mesafesi içindeki).

### 2. Nokta Kategorileri

Algoritma, her veri noktasını bu parametreleri kullanarak aşağıdaki üç tipten birine atar:

* **🟢 Çekirdek Noktalar (Core Points):**
    * Bir noktanın, kendi `Eps` mesafesi içinde kendisi hariç en az `MinPts` kadar komşusu varsa, bu nokta bir **çekirdek noktadır**.
    * Bu noktalar, bir kümenin kalbini ve temelini oluşturur.
* **🔵 Sınır Noktaları (Border Points):**
    * Bir nokta, bir **çekirdek noktanın** `Eps` mesafesi içindeyse, ancak kendi başına çekirdek nokta olmak için yeterli komşuya sahip değilse, bu nokta bir **sınır noktasıdır**.
    * Bu noktalar, kümenin kenarlarını tanımlar.
* **⚪ Gürültü Noktaları (Noise Points):**
    * Bir kümenin parçası olmayan noktalardır. Yani, ne bir çekirdek noktadır ne de herhangi bir çekirdek noktanın `Eps` mesafesi içindedir.
    * Bunlar genellikle **aykırı değerler** olarak kabul edilir.

### 3. Küme Oluşturma Süreci

1.  Algoritma rastgele bir nokta seçer.
2.  Seçilen noktanın `Eps` mesafesindeki komşularını kontrol eder.
3.  Eğer nokta bir **çekirdek nokta** ise, yeni bir küme oluşturulur.
4.  Bu küme, çekirdek noktanın tüm komşularını (ve onların komşularını, onlar da çekirdek nokta olduğu sürece) içererek **yoğunlukla erişilebilir** (density-reachable) tüm noktaları kapsayacak şekilde genişletilir.
5.  İşlem, kümeye eklenebilecek yeni çekirdek nokta kalmayana kadar devam eder.
6.  Tüm noktalar ziyaret edilene kadar bu süreç tekrarlanır.

Bu yöntem, DBSCAN'ı özellikle dairesel olmayan veya iç içe geçmiş gibi görünen, düzensiz şekilli kümeleri tespit etmekte üstün kılar.

# 🔍 DBSCAN'ı Derinlemesine İnceleme: Ne Zaman Kullanılır?

DBSCAN (Density-Based Spatial Clustering of Applications with Noise), kümeleme dünyasında özel bir yere sahiptir. Yoğunluğa dayalı yapısı, onu K-Means gibi mesafe tabanlı yöntemlerden ayırır.

## 🎯 DBSCAN Ne Zaman Kullanılmalı?

DBSCAN'ın güçlü ve zayıf yönlerini anlamak, doğru kümeleme algoritmasını seçmek için kritiktir.

### ✅ Avantajları ve En İyi Kullanım Alanları

| Alan | Açıklama | Neden DBSCAN İyi Bir Seçim? |
| :--- | :--- | :--- |
| **Rastgele Şekilli Kümeler** | **Dairesel olmayan**, elips, ay-şekilli (moon-shaped) veya iç içe geçmiş kümelere sahip veri setleri. | Yoğunluk tanımını kullandığı için, kümenin şekli Euclid mesafesiyle sınırlı değildir. |
| **Aykırı Değer Tespiti (Outlier Detection)** | Veri setinde doğal olarak **gürültülü** veya **anormal** noktaların bulunması. | DBSCAN, kümelere ait olmayan noktaları otomatik olarak **Gürültü Noktası (Noise Point)** olarak etiketler. |
| **Küme Sayısının (k) Bilinmemesi** | Önceden kaç tane küme olduğunu bilmediğiniz veya tahmin edemediğiniz durumlar. | Algoritma, yoğun bölgeleri kendiliğinden keşfederek küme sayısını veri setinden çıkarır. |

### ❌ Sınırlamaları ve Dikkat Edilmesi Gerekenler

| Sınırlama | Açıklama | Sonuç |
| :--- | :--- | :--- |
| **Parametre Seçimine Duyarlılık** | **`eps`** ve **`min_samples`** (MinPts) parametrelerinin seçimine karşı aşırı duyarlıdır. | Parametrelerdeki küçük bir değişiklik, kümeleme sonuçlarını dramatik bir şekilde değiştirebilir. |
| **Değişen Yoğunluktaki Kümeler** | Veri setinde bir kümenin çok yoğun, diğerinin ise çok seyrek olduğu durumlar. | Tek bir `eps` ve `min_samples` çifti, her iki farklı yoğunluktaki kümeyi de doğru şekilde tanımlayamaz. |
| **Yüksek Boyutlu Veriler** | **Çok yüksek boyutlu (high-dimensional)** veri setlerinde (Örn: 100+ özellik). | Yüksek boyutlarda, mesafeler anlamını yitirmeye başlar ("Curse of Dimensionality"). Yoğunluk tabanlı algoritmalar için bu bir zorluktur. |

---

## 🆚 Kümeleme Algoritmaları Karşılaştırma Tablosu

Aşağıdaki tablo, K-Means, Hiyerarşik Kümeleme (HC) ve DBSCAN'ı temel özellikler açısından karşılaştırmaktadır.

| Özellik | K-Means | Hiyerarşik Kümeleme (HC) | DBSCAN |
| :--- | :--- | :--- | :--- |
| **Temel Mekanizma** | Mesafe (Centroid'e olan uzaklık) | Bağlantı (Küme merkezleri veya noktaları arasındaki mesafe) | **Yoğunluk** (Yoğun bölgeleri bulma) |
| **Gerekli Parametreler** | Küme Sayısı (**k**) | Küme Sayısı veya Kesme Mesafesi | **Eps** ve **MinPts** |
| **Küme Şekilleri** | Yalnızca **Dairesel/Küre** (Konveks) | Değişken, ancak genellikle dairesel | **Rastgele Şekiller** (Irregular Shapes) |
| **Aykırı Değerler (Outliers)** | Aykırı değerler kümeleri **bozabilir** (merkezi kaydırır) | Genellikle bir kümeye dahil edilir | **Gürültü** olarak otomatik etiketler (Çok güçlü) |
| **Veri Ön İşleme** | Ölçekleme (Standardizasyon) **Zorunlu** | Ölçekleme **Zorunlu** | Ölçekleme **Önerilir** |
| **Hesaplama Maliyeti** | Hızlı (**O(k \* n \* iter)**) | Yavaş (**O(n³)** veya **O(n²)**) | Orta Hızda (**O(n log n)** veya **O(n²)**) |




<img width="704" height="279" alt="image" src="https://github.com/user-attachments/assets/2bda90f0-9c82-4faa-89e9-eb32eb99a43a" />

# 📊 Kümeleme Algoritmaları Özeti ve Karşılaştırması

Kümeleme (Clustering), denetimsiz öğrenmenin (Unsupervised Learning) temel bir alanıdır ve veri noktalarını benzerliklerine göre gruplara ayırır. İşte en popüler üç kümeleme yaklaşımının detaylı bir özeti ve karşılaştırması.

---

## 💡 1. K-Means Kümeleme

**K-Means**, kümeleme algoritmalarının en popüler ve en hızlısıdır.

* **Temel Fikir:** Her veri noktasını, önceden belirlediğimiz $K$ sayısı kadar küme merkezinden (centroid) en yakında olana atamaktır.
* **Çalışma Prensibi:**
    1.  Rastgele $K$ sayıda merkez seçilir.
    2.  Her nokta en yakın merkeze atanır.
    3.  Her kümenin yeni ortalama pozisyonu hesaplanarak merkezler güncellenir.
    4.  Atama ve güncelleme adımları, merkezler artık önemli ölçüde değişmeyene kadar tekrarlanır.
* **Avantajları (✅):**
    * **⚡ Hızlıdır** ve büyük veri setlerinde çok iyi performans gösterir.
    * Uygulaması kolaydır.
* **Sınırlamaları (❌):**
    * Küme sayısının ($K$) **önceden bilinmesi** gerekir.
    * Küme şekillerinin yaklaşık **küresel (spherikal)** olduğunu varsayar.
    * Aykırı değerlere karşı hassastır, merkezleri kaydırabilir.

---

## 🌳 2. Hiyerarşik Kümeleme (Hierarchical Clustering - HC)

**HC**, kümeler arasında bir hiyerarşi (ağaç yapısı) kurarak çalışır ve sonuçları bir **dendrogram** adı verilen ağaç diyagramı ile görselleştirir.

* **Temel Fikir:** Küme hiyerarşisini ya noktaları birleştirerek (Agnes/Birleştirici) ya da tek bir büyük kümeyi bölerek (Divisive/Bölücü) oluşturur.
* **Çalışma Prensibi (Birleştirici/Agglomerative):**
    1.  Her veri noktası başlangıçta ayrı bir kümedir.
    2.  En yakın iki küme birleştirilir.
    3.  Tüm noktalar tek bir kümede toplanana kadar 2. adım tekrarlanır.
* **Avantajları (✅):**
    * Küme sayısının önceden **kesin olarak belirlenmesine gerek yoktur**; dendrogramdan kesme mesafesine göre belirlenir.
    * Küme yapısının **ilişkisini** (hangi kümenin hangisinden oluştuğunu) gösteren zengin bir görsel (dendrogram) sunar.
* **Sınırlamaları (❌):**
    * Büyük veri setleri için **yavaştır** (genellikle $O(n^3)$ veya $O(n^2)$ karmaşıklığa sahiptir).
    * Birleştirme veya bölme adımı geri alınamaz.

---

## 🌊 3. DBSCAN

**DBSCAN (Density-Based Spatial Clustering of Applications with Noise)**, kümeleri nokta yoğunluğuna göre tanımlar.

* **Temel Fikir:** Belirli bir alandaki (Eps) nokta sayısı belirli bir eşiği (MinPts) aşıyorsa, bu alan bir kümenin parçasıdır.
* **Çalışma Prensibi:**
    * **Çekirdek Nokta (Core Point):** Eps mesafesi içinde MinPts kadar komşusu olan nokta.
    * **Sınır Noktası (Border Point):** Bir çekirdek noktanın mahallesinde olan ancak kendisi çekirdek olmayan nokta.
    * **Gürültü Noktası (Noise Point):** Ne çekirdek ne de sınır noktası olan aykırı değer.
* **Avantajları (✅):**
    * **Rastgele şekilli** kümeleri bulabilir (küresel varsayım yoktur).
    * **Gürültü/Aykırı değerleri** otomatik olarak tespit eder ve kümelemeden çıkarır.
    * Küme sayısının önceden belirlenmesi gerekmez.
* **Sınırlamaları (❌):**
    * **Parametre seçimine (Eps, MinPts) çok duyarlıdır.**
    * Yoğunlukları birbirinden çok farklı olan kümelerde iyi çalışmaz.

---

## 🌟 Kümeleme Algoritmaları Karşılaştırma Tablosu

| Özellik | K-Means Kümeleme | Hiyerarşik Kümeleme (HC) | DBSCAN |
| :--- | :--- | :--- | :--- |
| **Temel Mekanizma** | **Centroid'e Uzaklık** | **Bağlantı/Hiyerarşi** | **Yoğunluk** |
| **Gerekli Parametreler** | Küme Sayısı (**K**) | Küme Sayısı Veya Kesme Mesafesi | **Eps** ve **MinPts** |
| **Küme Şekilleri** | Yalnızca **Küresel/Sferik** (Convex) | Değişken, ancak mesafeye dayalı | **Rastgele Şekiller** (Arbitrary Shapes) |
| **Aykırı Değer Etkisi** | Küme merkezlerini **kaydırarak bozar** | Genellikle bir kümeye dahil edilir | **Gürültü** olarak otomatik etiketler |
| **Hesaplama Maliyeti** | **Çok Hızlı** | **Yavaş** (Özellikle büyük veri setlerinde) | **Orta/Değişken Hız** |
| **Örnek Kullanım** | Pazar segmentasyonu, hızlı analiz | Taksonomi, genetik analiz | Coğrafi kümeleme (GIS), aykırı değer tespiti |
