...
| Kavram | Açıklama | Örnek Durum (Spam Tespiti) | Sonuç/Durum ||
| :--- | :--- | :--- | :--- | :--- |
...
| Metrik/Kavram | Açıklaması | Formül (Karışıklık Matrisi Terimleriyle) | Kullanım Alanı ve Önemi ||
| :--- | :--- | :--- | :--- | :--- |
...
| 🎯 Algoritma | 🧠 Temel Kullanım Amacı | ✨ Anahtar Özellikler (Seçim Avantajı) | ⚠️ Dikkat Edilmesi Gerekenler (Sınırlama) | 💡 Metafor / Gerçek Hayat Örneği | Emoji |
| :--- | :--- | :--- | :--- | :--- | :--- |
| **Lojistik Regresyon** (Logistic Regression) | İkili sonuç olasılığını tahmin etme ve doğrusal (lineer) ilişkileri belirleme. | Hızlı, **Yorumlanabilir (Interpretable)** ve düzenlidir. İyi bir **temel model (baseline)**'dır. | Doğrusal olmayan (non-linear) karmaşık ilişkileri yakalayamaz. | **Basit Filtre:** Bir e-postanın spam olup olmadığını hızlıca tahmin eden basit bir e-posta filtresi. | ⚖️💨 |
| **Karar Ağaçları** (Decision Trees) | Sezgisel ve kolay anlaşılır bir dizi kural ile sınıflandırma. | Görselleştirilebilir, **yorumlaması çok kolaydır**. Kategorik ve sayısal veriyi yönetebilir. | **Aşırı Uyuma (Overfitting) çok yatkındır** (yüksek varyans). Verideki küçük bir değişiklik tüm ağacı değiştirebilir. | **Aşamalı Süreç Yöneticisi:** Bir müdürün promosyon verilip verilmeyeceğini adımlar halinde belirlemesi. | 🌳❓ |
| **Rastgele Ormanlar** (Random Forests) | Yüksek doğruluk ve sağlamlık ile karmaşık veriler üzerinde tahmin yapma. | **Aşırı uyumu büyük ölçüde azaltır** ve genellemede güçlüdür. Özellik önemini (feature importance) dengeli gösterir. | "Kara Kutu" eğilimi vardır (yorumlaması zor). Tek bir ağaçtan daha yavaş ve daha fazla bellek gerektirir. | **Jüri Kararı:** Tek bir kişi yerine, rastgele seçilmiş 100 kişinin çoğunluk oyu ile karar vermek. | 🌲🌲🌲 |
| **Destek Vektör Makineleri** (SVM) | Karmaşık, yüksek boyutlu verilerde sınıflar arasında en iyi ayırım (Optimal Margin) bulma. | **Çekirdek Hilesi (Kernel Trick)** sayesinde doğrusal olmayan problemleri çözmede çok güçlüdür. Yüksek boyutlu veride etkilidir. | Büyük veri setlerinde yavaştır. Çekirdek ve hiperparametre ayarı zordur. | **En Geniş Yolu Çizmek:** Farklı sınıflardaki iki şehir arasındaki en geniş otoyolu (marjin) inşa etmek. | ✨📐 |
| **K-En Yakın Komşular** (kNN) | Lokal (yerel) örüntülerin önemli olduğu küçük veri setlerinde sınıflandırma. | Çok basit ve sezgiseldir. **Eğitim fazı yoktur** ("Tembel Öğrenme" - Lazy Learning). | Büyük veri setlerinde **çok yavaştır** (her tahmin için tüm veriyi tarar). Özelliklerin ölçeklendirilmesi zorunludur. | **Mahalle Dedikodusu:** Yeni komşunun mesleğini (etiketini) belirlemek için en yakın 5 komşuya ne iş yaptıklarını sormak. | 🧑‍🤝‍🧑🐌 |



# 🧭 Hangi Algoritmayı Ne Zaman Seçmeli?

Doğru algoritmayı seçmek, veri setinizin özelliklerine ve iş hedeflerinize bağlıdır. Aşağıdaki kılavuz, farklı senaryolarda hangi modelin daha uygun olabileceğini göstermektedir.

---

## 2. 🚦 Karar Verme Kılavuzu

| Senaryo | Kritik Kriter | Önerilen Model(ler) | Emoji |
| :--- | :--- | :--- | :--- |
| **Hız ve Şeffaflık** | Modeli teknik olmayan kişilere kolayca açıklamak gerekiyor (Yorumlanabilirlik). | **Lojistik Regresyon** veya **Karar Ağaçları** (sığ derinlikte). | 🗣️⏱️ |
| **En İyi Performans** | Doğruluk ve sağlamlık en önemli şey (yorumlanabilirlik ikinci planda). | **Rastgele Ormanlar** (genellikle ilk denenir) veya ayarlanmış **SVM**. | 🏆🚀 |
| **Doğrusal Olmayan Veri** | Verideki ilişkiler karmaşık ve düz bir çizgiyle (lineer) ayrılamaz. | **Rastgele Ormanlar**, **SVM** (çekirdekli), **Karar Ağaçları**. | 〰️💡 |
| **Yüksek Boyut (Çok Özellik)** | Çok sayıda kolon (feature) var (özellik sayısı fazla). | **SVM** (etkili), **Rastgele Ormanlar**. | 📏🔬 |
| **Veri Boyutu Çok Büyük** | Modelin hızlı eğitilmesi ve tahmin yapması gerekiyor. | **Lojistik Regresyon**. (kNN ve SVM kaçınılmalıdır). | 💾💨 |


# 👑 Algoritma Seçimi Kılavuzu: Modelleri Toparlama

Artık Lojistik Regresyon, Karar Ağaçları, Rastgele Ormanlar, SVM ve kNN gibi temel sınıflandırma algoritmalarını biliyorsunuz. Unutmayın: Tek bir "en iyi" algoritma yoktur. En doğru seçim, daima verinize, iş problemine ve yapmanız gereken ödünlere bağlıdır.


# 1. 📋 Model Özeti: Kullanım Amaçları ve Temel Özellikler

Aşağıdaki tablo, öğrendiğiniz temel sınıflandırma algoritmalarının güçlü ve zayıf yönlerini, ne zaman kullanılmaları gerektiği bilgisiyle özetlemektedir.

---

| 🤖 Algoritma | 🥇 En İyi Uygulama Alanı | ➕ Güçlü Yönler (Pros) | ➖ Zayıf Yönler (Cons) | 💡 Ne Zaman Kullanılmalı? |
| :--- | :--- | :--- | :--- | :--- |
| **Lojistik Regresyon** ⚖️ | Basit, doğrusal (lineer) ilişkilerin olduğu veriler. | **Yorumlanabilir (Interpretable)**, hızlı, iyi bir **temel model (baseline)**. | Karmaşık, doğrusal olmayan (non-linear) sınırlarla zorlanır. | Hızlı, yorumlanabilir bir modele ihtiyacınız olduğunda veya performans testi için **temel oluşturmak** istediğinizde. |
| **Karar Ağaçları** 🌳 | Doğrusal olmayan (non-linear) ilişkileri yakalama. | **Kolay yorumlanır**, kategorik + sayısal veriyi yönetir, ölçeklendirme gerekmez. | **Aşırı uyuma (overfitting) çok yatkındır** (yüksek varyans). | Yorumlanabilir, esnek bir model istediğinizde, ancak **aşırı uymadan kaçınmaya** dikkat edin. |
| **Rastgele Ormanlar** 🌲 | Karışık özelliklere sahip karmaşık veri setleri. | **Aşırı uyumu azaltır**, sağlam (robust) ve genellikle ek ayar gerektirmeden iyi performans verir. | **Daha az yorumlanabilir**, çok büyük veri setlerinde yavaşlayabilir. | Ayar gerektirmeyen, **güvenilir ve güçlü** bir model istediğinizde. |
| **Destek Vektör Makineleri** 📏 | Sınıflar arasında net bir ayırım marjini olan veriler (özellikle yüksek boyutlu). | Yüksek boyutlu verilerle iyi çalışır, çekirdekler (kernels) ile güçlü performans sağlar. | Hesaplama açısından ağırdır (özellikle büyük veri setlerinde), ayarı zordur. | Karmaşık ancak büyük olmayan veri setlerinde **yüksek performans** istediğinizde. |
| **K-En Yakın Komşular** 🧑‍🤝‍🧑 | Lokal örüntülerin olduğu küçük veri setleri. | Basit, eğitim fazı yoktur ("Tembel Öğrenme"), veriye doğal olarak adapte olur. | Büyük veri setlerinde **çok yavaştır**, ölçeklendirmeye ve gürültüye karşı hassastır. | Küçük problemler için veya **yorumlanabilirliğin kritik olmadığı** durumlarda. |

# 2. 🧭 Karar Verme Kriterleri
